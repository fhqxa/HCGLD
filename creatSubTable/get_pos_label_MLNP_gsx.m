...
%% 
function [pos_label_set] = get_pos_label_MLNP_gsx(tree, cur_node)
cur_ancestor = tree_Ancestor_gsx(tree, cur_node);
cur_descendants = tree_Descendant_gsx(tree, cur_node);
leaf_nodes_set = tree_LeafNode_gsx(tree);
if (isempty(cur_ancestor))  % If the node is root node
    ind = find(tree(:,1) == cur_node);
    root_children_node = ind(1);
    all_pos_nodes = tree_Descendant_gsx(tree, root_children_node);
    index_root_ret = 1;
...
elseif (isempty(cur_descendants))   % the current node is a leaf node
    ...
    all_pos_nodes = tree_Descendant_gsx(tree, cur_node);
