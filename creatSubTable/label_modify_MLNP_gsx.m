...
function[label_mod] = label_modify_MLNP_gsx(labelSet, cur_node, tree)
label_mod = labelSet;
% Find the corresponding leaf nodes of every child node
children_set = get_children_set_gsx(tree, cur_node);   % Direct children nodes of the current node
for c =1:length(children_set)
    pos_label_set = get_pos_label_MLNP_gsx(tree, children_set(c));  % Relative leaf nodes of the children nodes
